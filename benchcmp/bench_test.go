...
var (
	...
	errWASMInit   error
	wasmInitMode  string
	wasmInitMu    sync.Mutex
)

func ensureWASMInitialised(tb testing.TB) {
	tb.Helper()
	ensureWASMInitialisedWithMode(tb, "default")
}

func ensureWASMMmapInitialised(tb testing.TB) {
	tb.Helper()
	if os.Getenv("YARAX_BENCH_WASM_MMAP") == "" {
		tb.Skip("set YARAX_BENCH_WASM_MMAP=1 to run mmap allocator benchmarks")
	}
	ensureWASMInitialisedWithMode(tb, "mmap")
}

func ensureWASMInitialisedWithMode(tb testing.TB, mode string) {
	tb.Helper()
	wasmInitMu.Lock()
	if wasmInitMode != "" && wasmInitMode != mode {
		currentMode := wasmInitMode
		wasmInitMu.Unlock()
		tb.Fatalf("WASM benchmarks with allocator %q must run in a separate go test process; current process already initialised with %q", mode, currentMode)
	}
	if wasmInitMode == "" {
		wasmInitMode = mode
	}
	...
	wasmInitOnce.Do(func() {
		...
		case "mmap":
			errWASMInit = wasmbind.Initialise(experimental.UseMmapMemoryAllocator())
		default:
			errWASMInit = os.ErrInvalid
		}
	})
	if errWASMInit != nil {
		tb.Fatalf("initialise wasm bindings (%s): %v", mode, errWASMInit)
	}
}
