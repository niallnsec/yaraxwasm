...
func GuestWASMPath(path string) InitialiseOption {
	return initialiseOptionFunc(func(cfg *initialiseConfig) error {
		if path == "" {
			return errors.New("guest wasm path is empty")
		}
		if cfg.hasSource {
			return errors.New("multiple guest wasm sources configured")
		}
...
func GuestWASMReader(r io.Reader) InitialiseOption {
	return initialiseOptionFunc(func(cfg *initialiseConfig) error {
		if r == nil {
			return errors.New("guest wasm reader is nil")
		}
		if cfg.hasSource {
			return errors.New("multiple guest wasm sources configured")
		}
...
func WithMemoryAllocator(allocator wazeroexperimental.MemoryAllocator) InitialiseOption {
	return initialiseOptionFunc(func(cfg *initialiseConfig) error {
		if allocator == nil {
			return errors.New("guest memory allocator is nil")
		}
