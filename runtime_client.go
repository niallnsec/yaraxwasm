...
import (
	...
	"github.com/niallnsec/yaraxwasm/internal/module"
...
func loadGuestWasm() ([]byte, error) {
	guestWasmOnce.Do(func() {
		...
		switch {
		case source.path != "":
			//nolint:gosec // Guest WASM override paths are intentionally caller-controlled.
...
func ensureUint64Stack(stack []uint64, size int) []uint64 {
	if cap(stack) < size {
...
func callStackLen(fn api.Function, paramCount int) int {
	stackLen := paramCount
...
func (c *guestClient) callFunctionLocked(fn api.Function, params []uint64) ([]uint64, error) {
	resultCount := len(fn.Definition().ResultTypes())
	c.callStack = ensureUint64Stack(c.callStack, callStackLen(fn, len(params)))
	copy(c.callStack, params)
	if err := fn.CallWithStack(c.ctx, c.callStack); err != nil {
		return nil, err
	}
	return c.callStack[:resultCount], nil
}

func (c *guestClient) call(name string, params ...uint64) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	_, err = c.callFunctionLocked(fn, params)
	return err
}
...
func (c *guestClient) callU32(name string, params ...uint64) (uint32, error) {
	...
	out, err := c.callFunctionLocked(fn, params)
	if err != nil {
		return 0, err
	}
	if len(out) != 1 {
...
func (c *guestClient) alloc(size, align uint32) (uint32, error) {
	return c.callRealloc(0, 0, align, size)
}

func (c *guestClient) callRealloc(ptr, oldSize, align, newSize uint32) (uint32, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	c.callStack = ensureUint64Stack(c.callStack, callStackLen(c.realloc, 4))
	c.callStack[0] = uint64(ptr)
	c.callStack[1] = uint64(oldSize)
	c.callStack[2] = uint64(align)
	c.callStack[3] = uint64(requestSize)
	if err := c.realloc.CallWithStack(c.ctx, c.callStack); err != nil {
		return 0, err
	}
	if len(c.realloc.Definition().ResultTypes()) != 1 {
		return 0, fmt.Errorf("allocator returned %d values, expected 1", len(c.realloc.Definition().ResultTypes()))
	}
	ptr, err := u32FromUint64(c.callStack[0], "guest allocator result")
	if err != nil {
		return 0, err
	}
	if newSize != 0 && ptr == 0 {
...
func (c *guestClient) reallocBuffer(ptr, oldSize, newSize, align uint32) (uint32, error) {
	if ptr == 0 {
		return c.alloc(newSize, align)
	}
	return c.callRealloc(ptr, oldSize, align, newSize)
}
...
func (c *guestClient) free(ptr, oldSize, align uint32) {
	...
	_, _ = c.callRealloc(ptr, oldSize, align, 0)
}

func (c *guestClient) allocAndWrite(data []byte) (uint32, uint32, error) {
	dataLen, err := u32FromLen(len(data), "guest write length")
	if err != nil {
		return 0, 0, err
	}
	ptr, err := c.alloc(dataLen, 1)
	if err != nil {
		return 0, 0, err
	}
	c.mu.Lock()
...
func (c *guestClient) withBufferView(handle uint32, fn func([]byte) error) error {
	...
	defer func() {
		_ = c.call(guestExportBufferDestroy, uint64(handle))
	}()
