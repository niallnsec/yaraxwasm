...
func initialiseExperimentalAllocator(t *testing.T) {
	...
	experimentalInitOnce.Do(func() {
		errExperimentalInit = yaraxwasm.Initialise(experimental.UseMmapMemoryAllocator())
	})
	if errExperimentalInit != nil && !strings.Contains(errExperimentalInit.Error(), "already locked after initialization") {
		require.NoError(t, errExperimentalInit)
	}
}
...
func maybeStartTimedProfiles(t *testing.T) {
	...
	if cpuPath != "" {
		//nolint:gosec // The profile output path is intentionally caller-controlled for diagnostics.
	...
	if tracePath != "" {
		//nolint:gosec // The trace output path is intentionally caller-controlled for diagnostics.
...
func TestScanReaderAtWithExperimentalUserfaultfdAllocator(t *testing.T) {
	...
	rules, err := yaraxwasm.Compile(`rule t { strings: $a = "readerat-mapped-sentinel" condition: $a }`)
	require.NoError(t, err)
	defer rules.Destroy()
...
func TestScanReaderAtWithPatternedReaderMatchesExpectedOffset(t *testing.T) {
	...
	const totalSize = 30 << 20
	pattern := []byte("readerat-random-offset-match")
	maxInsertOffset := big.NewInt(totalSize - int64(len(pattern)) + 1)
	insertOffsetN, err := rand.Int(rand.Reader, maxInsertOffset)
	require.NoError(t, err)
	insertOffsetU := insertOffsetN.Uint64()
	insertOffset := insertOffsetN.Int64()
	...
	rules, err := yaraxwasm.Compile(`rule t { strings: $a = "readerat-random-offset-match" condition: $a }`)
	require.NoError(t, err)
	defer rules.Destroy()
	...
	require.EqualValues(t, insertOffset, patternMatches[0].Offset())

	calls, minReqLen, maxReqLen, partialCalls := reader.stats()
	expectedMinChunkCalls := ((insertOffsetU + uint64(len(pattern)) - 1) / uint64(testUffdChunkSize)) + 1
	expectedMaxChunkCalls := (uint64(totalSize) + uint64(testUffdChunkSize) - 1) / uint64(testUffdChunkSize)
	t.Logf(
	...
	require.GreaterOrEqual(t, calls, expectedMinChunkCalls)
	require.LessOrEqual(t, calls, expectedMaxChunkCalls)
	...
	require.GreaterOrEqual(t, uffdStats["faults"], expectedMinChunkCalls)
	require.LessOrEqual(t, uffdStats["faults"], expectedMaxChunkCalls)
