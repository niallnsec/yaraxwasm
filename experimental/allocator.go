...
// [github.com/niallnsec/yaraxwasm.GuestFileMapper].
func (a *MmapMemoryAllocator) MapFileToGuest(mem api.Memory, guestOffset, mappedLength uint32, path string) (yaraxwasm.GuestMappedRegion, error) {
...
// [github.com/niallnsec/yaraxwasm.GuestReaderAtMapper].
func (a *MmapMemoryAllocator) MapReaderAtToGuest(mem api.Memory, guestOffset, mappedLength uint32, src io.ReaderAt, size int64) (yaraxwasm.GuestMappedRegion, error) {
...
func (a *MmapMemoryAllocator) Allocate(capacity, maxCapacity uint64) wazeroexperimental.LinearMemory {
	reserveBytes := reserveLength(capacity, maxCapacity)
	backend := a.backend
	if backend == nil {
		backend = mmapBackend{}
	}
...
// [github.com/niallnsec/yaraxwasm.InitialiseOption] that enables
// the experimental mmap-backed guest memory allocator.
func UseMmapMemoryAllocator() yaraxwasm.InitialiseOption {
	return yaraxwasm.WithMemoryAllocator(NewMmapMemoryAllocator())
}

func reserveLength(capacity, maxCapacity uint64) uint64 {
	if maxCapacity == 0 {
...
func (a *MmapMemoryAllocator) guestWindowAddress(mem api.Memory, guestOffset, mappedLength uint32) (unsafe.Pointer, error) {
	if mappedLength == 0 {
		...
		//nolint:nilnil // Zero-length mappings intentionally have no backing address.
		return nil, nil
	}
