...
func ReadFrom(r io.Reader) (*Rules, error) {
	...
	return readRulesFromBytes(data)
}

func readRulesFromBytes(data []byte) (*Rules, error) {
	...
	defer func() {
		_ = client.call("go_yrx_rules_destroy", uint64(handle))
	}()
...
func decodeMetadataValue(raw metadataValueJSON) (any, error) {
	switch raw.Kind {
	case "i":
