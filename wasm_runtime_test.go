...
func (m *testLinearMemory) Reallocate(size uint64) []byte {
	sizeLen, err := intFromUint64(size, "test linear memory size")
	if err != nil {
		panic(err)
	}
...
func (a *countingMemoryAllocator) Allocate(capacity, maxCapacity uint64) wazeroexperimental.LinearMemory {
	a.count++
	_ = maxCapacity
...
func (a *fakeFileMappingAllocator) MapFileToGuest(mem api.Memory, guestOffset, mappedLength uint32, path string) (GuestMappedRegion, error) {
	...
	requireLen, err := u32FromLen(len(data), "fake mapped file size")
	if err != nil {
		return nil, err
	}
	if mappedLength < requireLen {
		return nil, errors.New("mapped guest window too small")
	}
	if !mem.Write(guestOffset, data) {
		return nil, errors.New("write fake mapped file to guest memory failed")
	}
...
func (a *fakeReaderAtMappingAllocator) MapReaderAtToGuest(mem api.Memory, guestOffset, mappedLength uint32, src io.ReaderAt, size int64) (GuestMappedRegion, error) {
	...
	dataSize, err := intFromUint64(uint64(size), "fake reader mapping size")
	if err != nil {
		return nil, err
	}
	...
	dataLen, err := u32FromLen(len(data), "fake mapped reader size")
	if err != nil {
		return nil, err
	}
	if mappedLength < dataLen {
		return nil, errors.New("mapped guest window too small")
	}
	if !mem.Write(guestOffset, data) {
		return nil, errors.New("write fake mapped reader to guest memory failed")
	}
	return nopMappedRegion{}, nil
}
...
func TestGuestABIBufferLifecycle(t *testing.T) {
	...
	defer func() { _ = client.call("go_yrx_compiler_destroy", uint64(compiler)) }()
	...
	defer func() { _ = client.call("go_yrx_rules_destroy", uint64(rules)) }()
...
func TestCallResultKeepsErrorBufferAfterInterleavedCall(t *testing.T) {
	...
	defer func() { _ = client.call("go_yrx_compiler_destroy", uint64(otherCompiler)) }()
...
func TestReadAndFreeBufferReturnsDetachedCopy(t *testing.T) {
	...
	defer func() { _ = client.call(guestExportBufferDestroy, uint64(handle2)) }()
...
func TestConcurrentScansAndCompilesStress(t *testing.T) {
	...
	for worker := range compileWorkers {
		wg.Add(1)
		go func(worker int) {
			defer wg.Done()
			for iter := range compileIterations {
