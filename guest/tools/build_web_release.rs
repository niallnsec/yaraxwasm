...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !built_wasm.exists() {
        return Err(format!("wasm artifact not found: {}", built_wasm.display()).into());
    }
...
fn parse_args(args: impl IntoIterator<Item = OsString>) -> Result<Config, Box<dyn Error>> {
    ...
    if memory64 {
        if target_overridden && target != MEMORY64_TARGET {
            return Err(
                format!("--memory64 requires --target {MEMORY64_TARGET}, got {target}").into(),
            );
        }
...
fn ensure_build_prerequisites(crate_dir: &Path, config: &Config) -> Result<(), Box<dyn Error>> {
    if let Some(toolchain) = config.toolchain.as_deref() {
        ensure_rustup_component(crate_dir, toolchain, "rust-src")?;
    }

    if !config.build_std {
        ensure_target_installed(crate_dir, config.toolchain.as_deref(), &config.target)?;
    }
...
fn build_guest_release(crate_dir: &Path, config: &Config) -> Result<(), Box<dyn Error>> {
    let mut cmd = Command::new("cargo");
    cmd.current_dir(crate_dir);
...
fn optimize_wasm_file(input: &Path, output: &Path, memory64: bool) -> Result<(), Box<dyn Error>> {
    if output.exists() {
