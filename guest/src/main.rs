...
const GO_YRX_ABI_VERSION: u32 = 8;
...
struct RulesInner {
    rules: yara_x::Rules,
    rule_indices: HashMap<String, HashMap<String, u32>>,
}
...
fn structured_error_result<T: Serialize + ?Sized>(code: YRX_RESULT, value: &T) -> u64 {
...
fn serialize_to_buffer<T: Serialize + ?Sized>(value: &T) -> Result<u32, serde_json::Error> {
...
unsafe extern "C" {
    ...
    fn go_yrx_console_write_message(guest_instance_id: u64, message_ptr: u32, message_len: u32);
}

#[link(wasm_import_module = "go:scan-data/host")]
unsafe extern "C" {
    #[link_name = "begin-scan-bytes"]
    fn go_yrx_begin_scan_bytes(guest_instance_id: u64, data_ptr: u32, data_len: u32);

    #[link_name = "end-scan-bytes"]
    fn go_yrx_end_scan_bytes(guest_instance_id: u64);
}
...
struct ActiveScanBytesGuard {
    guest_instance_id: u64,
}

impl ActiveScanBytesGuard {
    fn begin(guest_instance_id: u64, data_ptr: u32, data_len: u32) -> Self {
        unsafe {
            go_yrx_begin_scan_bytes(guest_instance_id, data_ptr, data_len);
        }

        Self { guest_instance_id }
    }
}

impl Drop for ActiveScanBytesGuard {
    fn drop(&mut self) {
        unsafe {
            go_yrx_end_scan_bytes(self.guest_instance_id);
        }
    }
}
...
unsafe fn bytes_from_ptr_len(ptr: u32, len: u32) -> Result<&'static [u8], String> {
    if len == 0 {
        return Ok(&[]);
    }
    if ptr == 0 {
...
unsafe fn string_from_ptr_len(ptr: u32, len: u32) -> Result<String, YRX_RESULT> {
    let bytes = bytes_from_ptr_len(ptr, len).map_err(|_| YRX_RESULT::YRX_INVALID_ARGUMENT)?;
    std::str::from_utf8(bytes)
        .map(|s| s.to_owned())
        .map_err(|_| YRX_RESULT::YRX_INVALID_UTF8)
}

unsafe fn optional_string_from_ptr_len(ptr: u32, len: u32) -> Result<Option<String>, YRX_RESULT> {
    if ptr == 0 && len == 0 {
        Ok(None)
    } else {
...
unsafe fn json_from_ptr_len(ptr: u32, len: u32) -> Result<JsonValue, u64> {
    ...
    serde_json::from_slice(bytes).map_err(|err| error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, err))
}
...
fn metadata_to_json((identifier, value): (&str, MetaValue<'_>)) -> MetadataJson {
    let value = match value {
        ...
        MetaValue::Bytes(v) => {
            ...
            MetadataValueJson::Bytes(base64::engine::general_purpose::STANDARD.encode(bytes))
        }
    };
...
fn build_rules_inner(rules: yara_x::Rules) -> Result<RulesInner, String> {
    let mut rule_indices: HashMap<String, HashMap<String, u32>> = HashMap::new();

    for (index, rule) in rules.iter().enumerate() {
        let rule_index = u32::try_from(index)
            .map_err(|_| format!("rule index {index} exceeds u32 address space"))?;
        rule_indices
            .entry(rule.namespace().to_string())
            .or_default()
            .insert(rule.identifier().to_string(), rule_index);
    }
...
fn matching_rules_to_binary(
    rules: &RulesInner,
    results: &yara_x::ScanResults<'_, '_>,
) -> Result<Vec<u8>, String> {
    let matched_rule_count = results.matching_rules().count();
    let matched_rule_count = u32::try_from(matched_rule_count)
        .map_err(|_| "matched rule count exceeds u32 address space".to_string())?;

    let mut out = Vec::with_capacity(4 + matched_rule_count as usize * 8);
    append_u32(&mut out, matched_rule_count);

    for rule in results.matching_rules() {
        let rule_index = rules
            .rule_indices
            .get(rule.namespace())
            .and_then(|namespace_rules| namespace_rules.get(rule.identifier()))
            .copied()
            .ok_or_else(|| {
                format!(
                    "failed to resolve matching rule {}:{} to a static rule index",
                    rule.namespace(),
                    rule.identifier()
                )
            })?;
        append_u32(&mut out, rule_index);

        for pattern in rule.patterns() {
            let match_count = pattern.matches().count();
            let match_count = u32::try_from(match_count).map_err(|_| {
                format!(
                    "pattern {} in rule {}:{} has too many matches",
                    pattern.identifier(),
                    rule.namespace(),
                    rule.identifier()
                )
            })?;
            append_u32(&mut out, match_count);

            for m in pattern.matches() {
                let range = m.range();
                append_u64(&mut out, range.start as u64);
                append_u64(&mut out, (range.end - range.start) as u64);
            }
        }
    }
...
fn matching_rules_result(rules: &RulesInner, results: &yara_x::ScanResults<'_, '_>) -> u64 {
    match matching_rules_to_binary(rules, results) {
...
pub unsafe extern "C" fn go_yrx_version() -> u64 {
    success_result(buffer_from_bytes(yara_x::VERSION.as_bytes().to_vec()) as u32)
}
...
pub unsafe extern "C" fn go_yrx_compiler_add_source_with_origin(
    ...
    let compiler = match mut_from_handle::<CompilerState>(compiler) {
        ...
        None => {
            return error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, "compiler handle is null");
        }
    };
    ...
    match compiler.inner.add_source(source) {
        ...
        Err(err) => structured_error_result(YRX_RESULT::YRX_SYNTAX_ERROR, &err),
    }
}
...
pub unsafe extern "C" fn go_yrx_compiler_add_linter_tags_allowed(
    ...
    compiler
        .inner
        .add_linter(yara_x::linters::tags_allowed(tags).error(err_on_fail != 0));
    success_result(0)
}
...
pub unsafe extern "C" fn go_yrx_compiler_add_linter_required_metadata(
    ...
    let ident = match string_from_ptr_len(ident_ptr, ident_len) {
        Ok(ident) => ident,
        Err(code) => {
            return error_result(code, "metadata identifier is not valid UTF-8");
        }
    };
...
fn define_compiler_global<T>(compiler: &mut CompilerState, ident: String, value: T) -> u64
where
...
pub unsafe extern "C" fn go_yrx_compiler_build(compiler: u32) -> u64 {
    ...
    let built = mem::replace(&mut compiler.inner, create_compiler(compiler.flags)).build();
    match build_rules_inner(built) {
        Ok(inner) => success_result(into_handle(RulesState {
            inner: Arc::new(inner),
        })),
        Err(err) => error_result(YRX_RESULT::YRX_INVALID_STATE, err),
    }
...
pub unsafe extern "C" fn go_yrx_compiler_emit_wasm_file(
    ...
    let emit_compiler = mem::replace(&mut compiler.inner, create_compiler(compiler.flags));
...
pub unsafe extern "C" fn go_yrx_rules_count(rules: u32) -> u64 {
    let rules = match ref_from_handle::<RulesState>(rules) {
        Some(rules) => rules,
        None => {
            return error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, "rules handle is null");
        }
    };
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_rules_deserialize(data_ptr: u32, data_len: u32) -> u64 {
    ...
    match yara_x::Rules::deserialize(data) {
        Ok(rules) => match build_rules_inner(rules) {
            Ok(inner) => success_result(into_handle(RulesState {
                inner: Arc::new(inner),
            })),
            Err(err) => error_result(YRX_RESULT::YRX_INVALID_STATE, err),
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_scanner_create(rules: u32, guest_instance_id: u64) -> u64 {
    let rules = match ref_from_handle::<RulesState>(rules) {
        Some(rules) => rules,
        None => {
            return error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, "rules handle is null");
    ...
    let mut scanner = mem::transmute::<yara_x::Scanner<'_>, yara_x::Scanner<'static>>(
        yara_x::Scanner::with_runtime_session(&rules_arc.rules, guest_instance_id),
    );
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_block_scanner_create(rules: u32, guest_instance_id: u64) -> u64 {
    let rules = match ref_from_handle::<RulesState>(rules) {
        Some(rules) => rules,
        None => {
            return error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, "rules handle is null");
    ...
    let rules_arc = Arc::clone(&rules.inner);
    let mut scanner = mem::transmute::<blocks::Scanner<'_>, blocks::Scanner<'static>>(
        blocks::Scanner::with_runtime_session(&rules_arc.rules, guest_instance_id),
    );
    scanner.console_log(move |message| {
        emit_console_message(guest_instance_id, &message);
    });
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_scanner_set_timeout(scanner: u32, timeout_nanos: u64) -> u64 {
    let scanner = match mut_from_handle::<ScannerState>(scanner) {
        Some(scanner) => scanner,
        None => {
            return error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, "scanner handle is null");
        }
    };

    match scanner.inner_mut() {
        Ok(scanner) => {
            scanner.set_timeout(std::time::Duration::from_nanos(timeout_nanos));
            success_result(0)
        }
        Err(err) => error_result(YRX_RESULT::YRX_INVALID_STATE, err),
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_block_scanner_set_timeout(scanner: u32, timeout_nanos: u64) -> u64 {
...
fn set_scanner_global<T>(scanner: &mut ScannerState, ident: String, value: T) -> u64
where
    T: TryInto<yara_x::Variable, Error = yara_x::errors::VariableError>,
{
    match scanner.inner_mut() {
...
fn set_block_scanner_global<T>(scanner: &mut BlockScannerState, ident: String, value: T) -> u64
where
    T: TryInto<yara_x::Variable, Error = yara_x::errors::VariableError>,
{
    match scanner.inner_mut() {
...
pub unsafe extern "C" fn go_yrx_block_scanner_scan(
    ...
    match scanner.inner_mut() {
        Ok(scanner) => match scanner.scan(base, data) {
            Ok(_) => success_result(0),
            Err(yara_x::ScanError::Timeout) => pack_result(YRX_RESULT::YRX_SCAN_TIMEOUT, 0),
...
pub unsafe extern "C" fn go_yrx_block_scanner_finish(scanner: u32) -> u64 {
    ...
    let rules = Arc::clone(&scanner._rules);
    match scanner.inner_mut() {
        Ok(scanner) => match scanner.finish() {
            Ok(results) => matching_rules_result(rules.as_ref(), &results),
            Err(yara_x::ScanError::Timeout) => pack_result(YRX_RESULT::YRX_SCAN_TIMEOUT, 0),
            Err(err) => error_result(YRX_RESULT::YRX_SCAN_ERROR, err),
        },
        Err(err) => error_result(YRX_RESULT::YRX_INVALID_STATE, err),
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_scanner_scan(scanner: u32, data_ptr: u32, data_len: u32) -> u64 {
    let scanner = match mut_from_handle::<ScannerState>(scanner) {
        Some(scanner) => scanner,
        None => {
            return error_result(YRX_RESULT::YRX_INVALID_ARGUMENT, "scanner handle is null");
    ...
    let guest_instance_id = scanner.guest_instance_id;
    let _scan_bytes = ActiveScanBytesGuard::begin(guest_instance_id, data_ptr, data_len);
    let rules = Arc::clone(&scanner._rules);
    match scanner.inner_mut() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_scanner_slowest_rules_json(scanner: u32, n: u32) -> u64 {
        ...
        let profiling: Vec<ProfilingInfoJson> = match scanner.inner_ref() {
            Ok(scanner) => scanner
                ...
                .map(|item| ProfilingInfoJson {
                    ...
                    pattern_matching_time: item.pattern_matching_time.as_secs_f64(),
                    condition_exec_time: item.condition_exec_time.as_secs_f64(),
                })
                .collect(),
            Err(err) => {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn go_yrx_scanner_clear_profiling_data(scanner: u32) -> u64 {
    #[cfg(not(feature = "rules-profiling"))]
    {
        let _ = scanner;
        pack_result(YRX_RESULT::YRX_NOT_SUPPORTED, 0)
...
pub unsafe extern "C" fn cabi_realloc(
    ...
    if new_size == 0 {
        if !ptr.is_null() && old_size != 0 {
            let layout = Layout::from_size_align_unchecked(old_size, safe_align);
            dealloc(ptr, layout);
        }
        return ptr::null_mut();
    }
